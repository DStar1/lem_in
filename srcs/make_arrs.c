...
/*   Updated: 2018/01/31 20:23:05 by hasmith          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	make_arrs(t_mast *mast)
{
	...
	while (mast->file[mast->j])// <= mast->y_len)
	{
		...
		{
			// printf("len%d, end1:%s, mast->file[mast->j]: %s, y_len %d \n", ft_strlen(mast->file[mast->y_len - 1]),mast->file[mast->y_len - 1], mast->file[mast->j], mast->y_len);
			mast->room_arr[r] = ft_strsplit(mast->file[mast->j], ' ');
			// printf("end2:%s, y_len:%d, mast->j%d\n", mast->file[mast->y_len - 1], mast->y_len, mast->j);
			if (r == mast->start)
				printf("START:");
			if (r == mast->end)
				printf("END:");
